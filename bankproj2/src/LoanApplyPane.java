...
public class LoanApplyPane extends JPanel implements ActionListener, ItemListener{
	...
	public void initialize(){
		...
		JLabel lblDeposiKind = new JLabel("대출종류");
		...
		JLabel lblMoney = new JLabel("대출금액");
	...
	public void eventBtnSave() {
		...
		if(a_money.equals("")) {
			MsgBox.alert("대출 금액을 입력해주세요.");
			return;
		}
		if((!numCheck(a_money))){
			MsgBox.alert("대출금액은 숫자만 입력가능합니다.");
			return;
		}
		int a_amount = Integer.valueOf(a_money);
		if(a_amount <= 0) {
			MsgBox.alert("0원이상 입력해주세요.");
			return;
		}
		...
		if(check == 0) {
			...
			String sql ="insert into account(a_no, a_serial_no, a_date, a_amount, a_open_date, a_term, a_item_dist, a_item_name, a_b_no, a_c_no) "
					+ "values('" + a_no + "', 1, sysdate, " + a_amount + ", sysdate, '" + a_term + "', 'L1', '" + a_item_name + "', '" + a_b_no + "', '" + a_c_no + "')";
			DBControler db = new DBControler();
			db.connect();
			db.executeQuery(sql);
			db.disConnect();
	...
	public void setCbDepositKind() {
		...
		String sql = "select item_name, term1, term2, term3, rate1, rate2, rate3 from item where item_dist='L1'";
		DBControler db = new DBControler();
		db.connect();
		db.executeQuery(sql);
		ResultSet rs = db.getResultSet();
